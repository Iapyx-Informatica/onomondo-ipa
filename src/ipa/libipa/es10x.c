			...
			const char *function_name)
{
	...
	rc = ber_decode(0, IPA_ASN_TD_RW(td), (void **)&es10x_res_decoded, es10x_res_encoded->data,
			es10x_res_encoded->len);
	if (rc.code != RC_OK) {
		if (rc.code == RC_FAIL) {
		...
		ASN_STRUCT_FREE(*IPA_ASN_TD_RW(td), es10x_res_decoded);
		return NULL;
	}
				  ...
				  const char *function_name)
{
	...
	rc = der_encode(IPA_ASN_TD_RW(td), IPA_ASN_PTR_RW(es10x_req_decoded), ipa_asn1c_consume_bytes_cb,
			&es10x_req_encoded);
